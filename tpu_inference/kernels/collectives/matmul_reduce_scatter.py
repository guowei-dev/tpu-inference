...
def _matmul_reduce_scatter_kernel(
    ...
    a_vmem_ref,  # [m_per, n_per]  rows for this step's two chunks
    w_vmem_ref,  # [n_per, k_out]  resident
    ...
    def _do_w_local_copy(wait: bool = False):
        op = pltpu.make_async_copy(
            src_ref=w_hbm_ref.at[nc_slice, ko_slice],
            dst_ref=w_vmem_ref.at[nc_slice, ko_slice],
            sem=w_copy_sem,
        )
...
def matmul_reduce_scatter(
  ...
  Args:
    ...
    bnc: Contraction block size; must divide n // tp_size.
  ...
  """
    ...
    grid_spec = pltpu.PrefetchScalarGridSpec(
        ...
        scratch_shapes=(
            ...
            pltpu.VMEM((m_per_device, n_per_device), a.dtype),  # a rows
            pltpu.VMEM((n_per_device, k_out), w.dtype),  # w resident
    ...
    cost_estimate = pl.CostEstimate(flops=flops,
                                    ...
                                    transcendentals=0)
    vmem_bytes = (m_per_device * n_per_device * a.dtype.itemsize +
                  n_per_device * k_out * w.dtype.itemsize +
