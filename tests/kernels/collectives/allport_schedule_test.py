...
def simulate(num_dims):
    ...
    def execute(dv, op):
        ...
        if kind == "compute_tw":
            ...
            # the twin send reads run[l], which twin_merge later overwrites
            dv.open_sends[cell] = (op[1], bands)
            ...
            lbl = op[1]
            arrival = devs[dv.twin].snapshots[("twin", lbl)]
            check_write(dv, lbl, bands)
            dv.run[lbl] = {b: dv.own[lbl] + arrival for b in bands}
        elif kind == "p2_start":
            key = op[1]
            cell = ("p2", key)
            s, b, _ = key
...
class AllportScheduleTest(jtu.JaxTestCase):
    ...
    def test_simulated_execution(self, num_dims):
        sched, _, devs, violations = simulate(num_dims)
        tp = 2**(num_dims + 1)
