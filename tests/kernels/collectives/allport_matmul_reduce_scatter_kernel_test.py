...
Same numerics discipline as the hier/ring MM-RS tests: fp32 inputs make the
...
from tpu_inference.kernels.collectives import allport_matmul_reduce_scatter as amr
...
class AllportMatmulReduceScatterTest(jtu.JaxTestCase):
    ...
    def setUp(self):
        ...
        self.kernel = jax.jit(functools.partial(
            amr.allport_matmul_reduce_scatter, mesh=self.mesh,
            axis_name=AXIS))
    ...
    def test_rejects_bad_shapes(self):
        with self.assertRaisesRegex(ValueError, 'divisible by 128'):
            amr.validate_inputs(jnp.zeros((256, 64)), jnp.zeros((64, 100)), 8)
