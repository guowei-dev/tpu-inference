...
Same numerics discipline as the MM-RS sibling: fp32 inputs make the wire exact
(tight bound, carries the schedule-correctness burden); bf16 gets a
comparative bound vs the XLA path; 10 repetitions expose races — the risk
surface is the arrival-driven choreography, whose static schedule is
separately verified by the CPU simulator in allport_ag_schedule_test.py. The
lane-ragged case is covered because it exercises the n-block shape (128-aligned
blocks with the ragged remainder LAST) that the aligned shapes never reach.
"""
import functools
...
class AllportAllGatherMatmulTest(jtu.JaxTestCase):
    ...
    def test_bf16_no_worse_than_xla(self):
        x, y = _make_inputs(self.mesh, 2048, 1024, 2048, jnp.bfloat16)
        golden = _golden(x, y)
        out = jax.block_until_ready(self.kernel(x, y))
        ref = jax.block_until_ready(_xla_reference(self.mesh, x, y))
    ...
    def test_lane_ragged_n_per_device(self):
        """n // tp not a multiple of 128 — the n-block shape the aligned
        shapes never exercise."""
        n = 96 * self.tp  # n_per = 96, ragged
        x, y = _make_inputs(self.mesh, 1024, 512, n, jnp.float32)
        out = jax.block_until_ready(self.kernel(x, y))
        ref = jax.block_until_ready(_xla_reference(self.mesh, x, y))
        self.assertAllClose(out, ref, atol=1e-4, rtol=1e-4)

    def test_race_ten_runs_bitwise_identical(self):
        x, y = _make_inputs(self.mesh, 1024, 512, 2048, jnp.bfloat16)
        first = None
        for _ in range(10):
    ...
    def test_rejects_bad_shapes(self):
        with self.assertRaisesRegex(ValueError, 'multiple of'):
            aag.allport_all_gather_matmul(
                jnp.zeros((self.tp * 8, 512), jnp.bfloat16),
                jnp.zeros((512, self.tp * 128), jnp.bfloat16),
                self.mesh, AXIS)
